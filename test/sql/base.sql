...
SELECT facets.add_faceting_to_table('facetingtestsuite.documents',
        key => 'id',
        facets => array[
            facets.datetrunc_facet('created', 'month'),
            facets.datetrunc_facet('finished', 'month'),
            facets.plain_facet('category_id'),
            facets.array_facet('tags'),
            facets.bucket_facet('size', buckets => array[0,1000,5000,10000,50000,100000,500000]),
            facets.joined_plain_facet('author_id',
...
SELECT facets.populate_facets('facetingtestsuite.documents'::regclass);

SELECT * FROM facets.top_values('facetingtestsuite.documents'::regclass);

SELECT facets.add_facets('facetingtestsuite.documents',
    facets=>array[
        facets.plain_facet('type'),
        facets.joined_plain_facet('e.department',
...
SELECT facets.populate_facets_query('facetingtestsuite.documents'::regclass::oid);
...
-- Use the native merge_deltas function
SELECT merge_deltas_native('facetingtestsuite.documents'::regclass);
...
SELECT * FROM facets.count_results('facetingtestsuite.documents'::regclass,
                                     filters => array[row('category_id', 24)]::facets.facet_filter[]);
...
SELECT * FROM facets.top_values('facetingtestsuite.documents'::regclass, facets=>array['tags', 'type']);

SELECT facets.drop_facets('facetingtestsuite.documents', array['type', 'tags', 'not existing']);
SELECT * FROM facets.top_values('facetingtestsuite.documents'::regclass);

SELECT facets.drop_faceting('facetingtestsuite.documents');

-- Check that adding faceting back in works
SELECT facets.add_faceting_to_table('facetingtestsuite.documents',
        key => 'id',
        facets => array[facets.plain_facet('category_id')]);
SELECT * FROM facets.top_values('facetingtestsuite.documents'::regclass);
