...
BEGIN
    ...
    EXECUTE format($sql$
        ...
        CREATE INDEX %s_facet_lookup ON %s (facet_id, facet_value, chunk_id);
...
BEGIN
    ...
    RETURN QUERY EXECUTE format($sql$
        SELECT fd.facet_name, counts.facet_value, counts.sum::bigint, fd.facet_id FROM (
    ...
    IF p_filter_bitmap IS NULL THEN
        ...
        RETURN QUERY EXECUTE format($sql$
            SELECT
                %L AS facet_name,
                facet_value,
                rb_cardinality(postinglist) AS cardinality,
                %s AS facet_id
            FROM %s
            WHERE facet_id = %s
    ...
    ELSE
        ...
        RETURN QUERY EXECUTE format($sql$
            SELECT
                %L AS facet_name,
                facet_value,
                rb_and_cardinality(postinglist, $1) AS cardinality,
                %s AS facet_id
            FROM %s
            WHERE facet_id = %s
...
BEGIN
    ...
    -- For other facet types
    EXECUTE format($sql$
        SELECT rb_or_agg(postinglist)
        FROM %s
        WHERE facet_id = $1
        AND facet_value = $2
    $sql$,
        facets._qualified(tdef.schemaname, tdef.facets_table)
    ) INTO result USING v_facet_id, p_facet_value;
...
BEGIN
    ...
    EXECUTE format('
        ...
        AND facet_value = $2',
        facets._qualified(tdef.schemaname, tdef.facets_table)
    ) INTO result USING v_facet_id, v_facet_value;
...
BEGIN
    ...
    FOR i IN 0..jsonb_array_length(p_filters) - 1 LOOP
        ...
        EXECUTE format('
            ...
            AND facet_value = $2',
            facets._qualified(tdef.schemaname, tdef.facets_table)
        ) INTO facet_bitmap USING v_facet_id, v_facet_value;
...
SELECT
    jsonb_build_object(
        ...
        'levels', jsonb_build_array(
            jsonb_build_object('name', %L, 'items', COALESCE((SELECT level_items FROM level0_json), '[]'::jsonb)),
            jsonb_build_object('name', %L, 'items', COALESCE((SELECT level_items FROM level1_json), '[]'::jsonb)),
            jsonb_build_object('name', %L, 'items', COALESCE((SELECT level_items FROM level2_json), '[]'::jsonb))
        )
    ) AS hierarchy
$sql$,
            tdef.schemaname, tdef.facets_table, 
...
BEGIN
    ...
    RETURN combined_hierarchical_result;
END;
$$;
...
BEGIN
    ...
    -- Get the table ID for faceting
    SELECT oid INTO v_table_id
    FROM pg_class
    WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = p_schema_name)
    ...
    -- Apply facet filtering if facets are provided
    IF p_facets IS NOT NULL AND jsonb_typeof(p_facets) = 'object' AND p_facets != '{}'::jsonb THEN
        -- Get filtered document IDs
        SELECT array_agg(id) INTO v_filtered_ids
        FROM facets.filter_documents_by_facets(p_schema_name, p_facets, p_table_name) AS id;
    END IF;
    
    -- Build the search CTE based on whether vector search is enabled and facet filtering
    IF p_vector_column IS NOT NULL THEN
        -- Combined BM25 and vector search
        v_search_cte := format('
            WITH search_results AS (
            ...
            p_content_column, p_query,
            p_vector_column, p_query,
            CASE WHEN v_filtered_ids IS NOT NULL THEN format('AND id = ANY(%L)', v_filtered_ids) ELSE '' END
        );
    ELSE
        -- BM25 search only
        v_search_cte := format('
    ...
    -- Get document IDs from search results for facet calculation
    IF v_results IS NOT NULL AND jsonb_array_length(v_results) > 0 THEN
        -- Get hierarchical facets
        v_facets := facets.hierarchical_facets(
            v_table_id,
            p_facet_limit,
            NULL,
            ARRAY(
                SELECT (jsonb_array_elements(v_results)->>'id')::int
            )
        );
    ELSE
        -- No results, return empty facets
        v_facets := '[]'::jsonb;
    END IF;
