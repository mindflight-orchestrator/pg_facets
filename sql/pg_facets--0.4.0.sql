                    ...
                    IF v_filter_cardinality > 0 AND v_filter_cardinality < 100000 THEN
                        ...
                        EXECUTE format('
                            ...
                            SELECT 
                                ...
                                (SELECT COALESCE(rb_build_agg((id)::int4), rb_build(ARRAY[]::int[])) FROM filtered)
                        ',
                ...
                IF v_filter_bitmap IS NOT NULL THEN
                    ...
                    IF v_filter_cardinality > 0 AND v_filter_cardinality < 100000 THEN
                        ...
                        EXECUTE format('
                            ...
                            SELECT COALESCE(rb_build_agg((id)::int4), rb_build(ARRAY[]::int[]))
                            FROM search_results
                        ',
                            v_tdef.key,
                            p_schema_name, p_table_name,
...
BEGIN
    ...
    -- If pg_trgm isn't installed, disable fuzzy matching to avoid runtime errors.
    IF p_fuzzy_match AND NOT v_has_trgm THEN
        p_fuzzy_match := false;
    END IF;

    IF p_fuzzy_match AND v_has_trgm THEN
        RETURN QUERY
        WITH
        stats AS (
            SELECT total_documents::float8 AS total_documents,
                   avg_document_length::float8 AS avg_document_length
            FROM facets.bm25_statistics
            WHERE table_id = v_table_oid
        ),
        qterms AS (
            SELECT DISTINCT unnest(tsvector_to_array(to_tsvector(p_language::regconfig, p_query)))::text AS term
        ),
        expanded_terms AS (
            SELECT DISTINCT i.term_text
            FROM facets.bm25_index i
            JOIN qterms q ON TRUE
            WHERE i.table_id = v_table_oid
              AND (
                    i.term_text = q.term
                 OR (p_prefix_match AND i.term_text LIKE q.term || '%')
                 OR (i.term_text % q.term AND similarity(i.term_text, q.term) >= p_fuzzy_threshold)
              )
        ),
        candidate_docs AS (
            SELECT DISTINCT d.doc_id
            FROM facets.bm25_documents d
            WHERE d.table_id = v_table_oid
              AND EXISTS (
                  SELECT 1
                  FROM facets.bm25_index i
                  JOIN expanded_terms et ON et.term_text = i.term_text
                  WHERE i.table_id = v_table_oid
                    AND rb_contains(i.doc_ids, d.doc_id::int)
              )
        ),
        term_rows AS (
            SELECT
                d.doc_id,
                rb_cardinality(i.doc_ids)::float8 AS doc_count,
                COALESCE((i.term_freqs->>d.doc_id::text)::int, 0) AS tf
            FROM candidate_docs d
            JOIN facets.bm25_index i
              ON i.table_id = v_table_oid
            JOIN expanded_terms et
              ON et.term_text = i.term_text
            WHERE rb_contains(i.doc_ids, d.doc_id::int)
        )
        SELECT
            d.doc_id,
            COALESCE(SUM(
                ln((s.total_documents + 1.0) / (tr.doc_count + 0.5))
                * (tr.tf * (p_k1 + 1.0))
                / (tr.tf + p_k1 * (1.0 - p_b + p_b * (md.doc_length::float8 / NULLIF(s.avg_document_length, 0.0))))
            ), 0.0)::float AS score
        FROM candidate_docs d
        JOIN facets.bm25_documents md
          ON md.table_id = v_table_oid AND md.doc_id = d.doc_id
        JOIN term_rows tr
          ON tr.doc_id = d.doc_id
        CROSS JOIN stats s
        WHERE tr.tf > 0
          AND s.total_documents > 0
          AND s.avg_document_length > 0
        GROUP BY d.doc_id, md.doc_length, s.total_documents, s.avg_document_length
        HAVING COALESCE(SUM(
            ln((s.total_documents + 1.0) / (tr.doc_count + 0.5))
            * (tr.tf * (p_k1 + 1.0))
            / (tr.tf + p_k1 * (1.0 - p_b + p_b * (md.doc_length::float8 / NULLIF(s.avg_document_length, 0.0))))
    ...
    ELSE
        ...
        expanded_terms AS (
            ...
            WHERE i.table_id = v_table_oid
              AND (
                    (NOT p_prefix_match AND i.term_text = q.term)
                 OR (p_prefix_match AND i.term_text LIKE q.term || '%')
              )
        ),
        candidate_docs AS (
...
BEGIN
    ...
    RETURN COALESCE((
        ...
        docmeta AS (
            SELECT doc_length::float8 AS doc_length
            FROM facets.bm25_documents
            WHERE table_id = v_table_oid AND doc_id = p_doc_id
        ),
        qterms AS (
            SELECT DISTINCT unnest(tsvector_to_array(to_tsvector(p_language::regconfig, p_query)))::text AS term
        ),
        term_rows AS (
            SELECT
                rb_cardinality(i.doc_ids)::float8 AS doc_count,
                COALESCE((i.term_freqs->>p_doc_id::text)::int, 0) AS tf
            FROM facets.bm25_index i
            JOIN qterms q ON i.term_text = q.term
            WHERE i.table_id = v_table_oid
        )
        SELECT COALESCE(SUM(
            ln((s.total_documents + 1.0) / (tr.doc_count + 0.5))
            * (tr.tf * (p_k1 + 1.0))
            / (tr.tf + p_k1 * (1.0 - p_b + p_b * (d.doc_length / NULLIF(s.avg_document_length, 0.0))))
        ), 0.0)::float
        FROM term_rows tr
        CROSS JOIN stats s
        CROSS JOIN docmeta d
        WHERE tr.tf > 0
          AND s.total_documents > 0
          AND s.avg_document_length > 0
    ), 0.0);
...
BEGIN
    ...
    LOOP
        -- Index the document (uses incremental stats internally now)
...
-- Parallel batch indexing using multiple database connections
-- This function spawns parallel workers using dblink
...
BEGIN
    ...
    -- Spawn parallel workers using dblink_send_query (async)
    FOR i IN 1..p_num_workers LOOP
        v_conn_name := 'bm25_worker_' || i;
        
        -- Build worker query using the worker function
        v_worker_query := format(
            'SELECT * FROM facets.bm25_index_worker(%L::regclass, %L, %L, %L, %s, %s, %s)',
            p_table_id::text, p_source_query, p_content_column, p_language,
            v_total_docs, p_num_workers, i
        );
        
        BEGIN
            -- Connect and send async query
            PERFORM dblink_connect(v_conn_name, v_conn_string);
            PERFORM dblink_send_query(v_conn_name, v_worker_query);
        EXCEPTION WHEN OTHERS THEN
            -- If dblink not available, fall back to sequential
            worker_id := i;
            docs_indexed := 0;
            elapsed_ms := 0;
            status := 'dblink not available: ' || SQLERRM;
            RETURN NEXT;
            RETURN;
        END;
    END LOOP;
    ...
    FOR i IN 1..p_num_workers LOOP
        ...
        BEGIN
            ...
            FOR v_result IN SELECT * FROM dblink_get_result(v_conn_name) AS t(docs_indexed int, elapsed_ms float)
            LOOP
                worker_id := i;
